...
describe("Filter before the GraphiQL Toolbar Buttons", () => {
    ...
    beforeEach(() => {
        ...
        hooks.addFilter( 'graphiql_toolbar_before_buttons', 'test-filter', (before, props) => {
            return [...before, <button key="test-button" data-testid="test-button">Test Button</button>];
        })
    })
...
describe("Filter after the GraphiQL Toolbar Buttons", () => {
    ...
    beforeEach(() => {
        const { hooks } = wpGraphiQL;
        hooks.addFilter( 'graphiql_toolbar_after_buttons', 'test-filter', (before, props) => {
            return [...before, <button key="test-after-button" data-testid="test-after-button">Test Button</button>];
        })
    })
...
describe("Filter all Toolbar Buttons", () => {
    ...
    beforeEach(() => {
        const { hooks } = wpGraphiQL;
        hooks.addFilter( 'graphiql_toolbar_buttons', 'test-filter', (before, props) => {
            return [{
                key: "test-button",
                label: "Test Button",
                label: "Test Button",
